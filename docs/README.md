# Dask Gateway Pack docs (Astro + Starlight)

Astro-powered documentation site for `dask-gateway-pack`, built on Starlight
with the shared `@nebari/starlight` theme. Mirrors the pattern used by
`nebari-dev/provenance-collector-pack` (and `llm-serving-pack`): build on
Astro, deploy to Cloudflare Pages, and route through the `packs.nebari.dev`
portal.

Deployed at **`https://packs.nebari.dev/dask-gateway-pack/`** (the portal
Worker proxies that path to the `dask-gateway-pack` Cloudflare Pages
project). Pull requests get an ephemeral preview at
`https://<branch>.dask-gateway-pack.pages.dev`.
...
4. Computes `SITE`/`BASE` per event: on `main`, `BASE=/dask-gateway-pack/`
...
6. Deploys `docs/dist/` to the `dask-gateway-pack` Cloudflare Pages project
...
Portal routing at `packs.nebari.dev/dask-gateway-pack/` is driven by
...
slug to `https://dask-gateway-pack.pages.dev`.
...
- Guides: `quick-start.md`, `architecture.md`, `consuming-from-data-science-pack.md`.
- Reference: `configuration.md`, `nebariapp-crd-reference.md`, `roadmap.md`.
